...
namespace fast_planner
{
  ...
  void SDFMap::initMap(ros::NodeHandle &nh)
  {
    ...
    md_->virtual_walls = std::vector<VirtualWall>();
    md_->virtual_walls_voxel_ = std::vector<int>();
    md_->are_virtual_walls_enabled = true;
    md_->raycast_num_ = 0;
      ...
      const Eigen::Vector3d &camera_pos)
  {
    ...
    while (!md_->cache_voxel_.empty())
    {
      ...
      bool is_in_virtual_wall = false;
      if (md_->are_virtual_walls_enabled)
        for (auto &&address : md_->virtual_walls_voxel_)
          if (adr == address)
          {
            is_in_virtual_wall = true;
            break;
          }
      if (is_in_virtual_wall)
        continue;
  ...
  void SDFMap::addVirtualWall(
      const Eigen::Vector3d center,
      const Eigen::Vector3d dimensions,
      const Eigen::Quaterniond orientation)
  {
    VirtualWall virtual_wall;
    virtual_wall.center = center;
    virtual_wall.dimensions = dimensions;
    virtual_wall.orientation = orientation;
    auto first_point_vector = orientation * dimensions;
    auto second_point_vector = orientation * (-dimensions);
    auto first_point = center + first_point_vector;
    auto second_point = center + second_point_vector;
    virtual_wall.min_point[0] = std::min(first_point[0], second_point[0]);
    virtual_wall.min_point[1] = std::min(first_point[1], second_point[1]);
    virtual_wall.min_point[2] = std::min(first_point[2], second_point[2]);
    virtual_wall.max_point[0] = std::max(first_point[0], second_point[0]);
    virtual_wall.max_point[1] = std::max(first_point[1], second_point[1]);
    virtual_wall.max_point[2] = std::max(first_point[2], second_point[2]);
    md_->virtual_walls.push_back(virtual_wall);
    if (md_->are_virtual_walls_enabled)
    {
      const VirtualWall &virtual_wall_ref = virtual_wall;
      addWallToSdf(virtual_wall_ref);
    }
  }

  void SDFMap::addWallToSdf(const VirtualWall &wall)
  {
    double step = 0.2;
    double x = wall.min_point[0];
    for (double x = wall.min_point[0]; x < wall.max_point[0]; x += step)
      for (double y = wall.min_point[1]; y < wall.max_point[1]; y += step)
        for (double z = wall.min_point[2]; z < wall.max_point[2]; z += step)
        {
          md_->virtual_walls_voxel_.push_back(toAddress(x, y, z));
          md_->occupancy_buffer_[toAddress(x, y, z)] = mp_->clamp_max_log_;
        }
  }

  void SDFMap::enableVirtualWalls()
  {
    if (md_->are_virtual_walls_enabled)
      return;
    for (auto &&address : md_->virtual_walls_voxel_)
  ...
  void SDFMap::disableVirtualWalls()
  {
    if (!md_->are_virtual_walls_enabled)
      return;
    for (auto &&address : md_->virtual_walls_voxel_)
      md_->occupancy_buffer_[address] = mp_->clamp_min_log_;
    md_->are_virtual_walls_enabled = false;
  }
