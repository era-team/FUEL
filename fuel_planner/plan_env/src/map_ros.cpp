...
namespace fast_planner
{
  ...
  void MapROS::init()
  {
    ...
    add_wall_service = node_.advertiseService("/sdf_map/add_wall", &MapROS::add_wall_callback, this);
    set_are_walls_enabled_service = node_.advertiseService("/sdf_map/set_are_walls_enabled", &MapROS::setAreWallsEnabledCallback, this);
  ...
  bool MapROS::add_wall_callback(plan_env::AddWalls::Request &request, plan_env::AddWalls::Response &response)
  {
    auto walls = request.objects.detected_objects;
    for (int i = 0; i < walls.size(); i++)
    {
      ...
      center[0] = wall.pose.position.x;
      center[1] = wall.pose.position.y;
      center[2] = wall.pose.position.z;
      dimensions[0] = wall.dimensions.x;
      dimensions[1] = wall.dimensions.y;
      dimensions[2] = wall.dimensions.z;
      orientation.w() = wall.pose.orientation.w;
      orientation.x() = wall.pose.orientation.x;
      orientation.y() = wall.pose.orientation.y;
      orientation.z() = wall.pose.orientation.z;
      map_->addVirtualWall(center, dimensions, orientation);
    }
    return true;
  }

  bool MapROS::setAreWallsEnabledCallback(std_srvs::SetBoolRequest &request, std_srvs::SetBoolResponse &response)
  {
    if (request.data)
      map_->enableVirtualWalls();
    else
      map_->disableVirtualWalls();
    return true;
  }
